...
echo "MERGED_DIR=$MERGED_DIR"
  ...
  # -------------------------
  # Hygiene: deterministic relabels before quality gate + pointer update
  # - downgrade contradicts when no explicit negation cues
  # - downgrade depends_on when no explicit dependency cues
  # Writes a new field dir and then we treat that as MERGED_DIR.
  # -------------------------
  HYGIENE_DIR="$ROOT/field/hygiene_${RUN_TAG}_$(ts)"
  HYGIENE_LOG="/tmp/hygiene_${RUN_TAG}_$(ts).log"
  python3 "$ROOT/scripts/rff_hygiene_relabel_v0.py" \
    --field_dir "$MERGED_DIR" \
    --out_dir "$HYGIENE_DIR" \
    | tee "$HYGIENE_LOG"

  # Preserve cross-edge artifacts for audit tooling (if available)
  if [[ -n "${LINK_DIR:-}" && -f "$LINK_DIR/concept_edges_cross.jsonl" ]]; then
    cp -f "$LINK_DIR/concept_edges_cross.jsonl" "$HYGIENE_DIR/" || true
  fi
  if [[ -n "${SCORED_DIR:-}" && -f "$SCORED_DIR/concept_edges_cross_scored.jsonl" ]]; then
    cp -f "$SCORED_DIR/concept_edges_cross_scored.jsonl" "$HYGIENE_DIR/" || true
  fi

  MERGED_DIR="$HYGIENE_DIR"
  echo "MERGED_DIR=$MERGED_DIR (post-hygiene)"
