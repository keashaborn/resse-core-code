...
def main():
    ...
    out_dir = args.resume_from_dir or args.out_dir or f"/home/ubuntu/resse-core/fm_teacher/evals/passC_prompt_v0_{ts}"
    ...
    with open(ok_path, file_mode, encoding="utf-8", buffering=1) as okf, open(bad_path, file_mode, encoding="utf-8", buffering=1) as badf:
        ...
        with cf.ThreadPoolExecutor(max_workers=max_workers) as ex:
            futs = [ex.submit(_process_one, d, c) for d, c in work]
            completed = 0
            for fut in cf.as_completed(futs):
                rec, is_ok, st = fut.result()
                completed += 1

                cid = rec.get("cluster_id") or ""
                if cid:
                    done_ids.add(cid)
                ...
                if args.print_every and completed % int(args.print_every) == 0:
                    print(
                        f"passC_progress completed={completed}/{len(work)} ok={ok} bad={bad} "
                        f"done_total={len(done_ids)}/{total_expected}"
                    )
