...
def main():
    ...
    ap.add_argument("--run_tag", default="passM_v0")
    args = ap.parse_args()
    ...
    for rec in iter_jsonl(ok_path):
        ...
        if not isinstance(canonical_i, int) or canonical_i not in facts:
            # Shouldn't happen in ok.jsonl, but guard anyway.
            continue

        canonical_text = facts.get(canonical_i, "") or ""
        concept_id = sha1(f"{domain}||{norm_text(canonical_text)}")
        ...
        # Node record (dedupe + merge provenance)
        if concept_id not in node_by_id:
            node_by_id[concept_id] = {
            ...
            if cluster_id not in node_by_id[concept_id]["source_cluster_ids"]:
                node_by_id[concept_id]["source_cluster_ids"].append(cluster_id)
        ...
        # Edges (local micrograph edges attached to this concept/cluster)
        for e in (obj.get("edge_candidates") or []):
            ...
            edge_rows.append({
                ...
                "dst_i": di,
                "support_i_list": sup,
                "src_text": facts.get(si, "") if isinstance(si, int) else "",
                "dst_text": facts.get(di, "") if isinstance(di, int) else "",
            })
    ...
    with summary_out.open("w", encoding="utf-8") as f:
        f.write("# PassM materialization summary\n\n")
